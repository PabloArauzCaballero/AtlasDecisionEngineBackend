...
describe("frontend read endpoints", () => {
        ...
        [
          {
            ...
            steps: [
              { stepOrder: 1, requiredRole: "QA_ANALYST", status: "PENDING" },
            ],
          },
        ],
        1,
  ...
  it("checks the suite tenant before returning test cases", async () => {
    ...
    const findCases = jest
      .fn()
      .mockResolvedValue([{ id: 3n, caseCode: "CASE-001" }]);
    ...
    await expect(service.listCases(7n, 12n)).resolves.toEqual([
      { id: 3n, caseCode: "CASE-001" },
    ]);
