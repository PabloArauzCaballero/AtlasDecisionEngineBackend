...
// del realm exterior. Antes se hacia con setPrototypeOf(copy, null), que servia para
// objetos pero sobre un ARRAY le arrancaba Array.prototype: el script recibia algo
// sin .map, .reduce ni .sort, de modo que cualquier variable de tipo LISTA era
// inservible y moria con SCRIPT_EXECUTION_FAILED sin explicar la causa.
//
// Este archivo es la copia que ejecuta el SIDECAR (SCRIPT_RUNNER_MODE=SIDECAR); la
// del proceso de la API vive en script-node-runner.service.ts. Las dos deben
// cambiar a la vez: arreglar solo una deja el defecto vivo en el modo que se usa.
