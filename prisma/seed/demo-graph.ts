...
export interface ConditionDefinition { code: string; name: string; expression: unknown }
export interface ActionDefinition { code: string; type: string; payload: Record<string, unknown>; terminal: boolean; reason?: string }
export interface NodeDefinition { key: string; type: string; label: string; order: number; terminal: boolean; config: Record<string, unknown> }
export interface EdgeDefinition { key: string; from: string; to: string; priority: number; default: boolean; condition?: string }
...
export async function buildDemoGraph(
  ...
  const conditionDefinitions: ConditionDefinition[] = [
    ...
    { code: 'FRAUD_CLEAR', name: 'Sin señal crítica de fraude', expression: { op: 'eq', left: { var: 'fraud_signal' }, right: { value: false } } },
    { code: 'ELIGIBLE_AGE', name: 'Mayoría de edad', expression: { op: 'gte', left: { var: 'age' }, right: { value: 18 } } },
    { code: 'RISK_ACCEPTABLE', name: 'Riesgo dentro del umbral', expression: { op: 'gte', left: { var: 'bureau_score' }, right: { value: 550 } } },
  ];
  ...
  const actionDefinitions: ActionDefinition[] = [
    { code: 'SET_APPROVED', type: 'SET_OUTCOME', payload: { outcome: 'APPROVED' }, terminal: false, reason: 'APPROVED_POLICY' },
    {
      ...
      payload: {
        valueExpression: {
          op: 'round',
          arg: { op: 'min', args: [{ value: 5000 }, { op: 'mul', args: [{ var: 'monthly_income' }, { value: 0.35 }] }] },
          precision: 2,
        },
      },
      terminal: false,
    ...
    { code: 'DECLINE_KYC', type: 'SET_OUTCOME', payload: { outcome: 'DECLINED' }, terminal: true, reason: 'KYC_OR_CONSENT_INVALID' },
    { code: 'DECLINE_AGE', type: 'SET_OUTCOME', payload: { outcome: 'DECLINED' }, terminal: true, reason: 'AGE_NOT_ELIGIBLE' },
    { code: 'DECLINE_RISK', type: 'SET_OUTCOME', payload: { outcome: 'DECLINED' }, terminal: true, reason: 'RISK_THRESHOLD_NOT_MET' },
    {
      code: 'REVIEW_FRAUD',
  ...
  const nodeDefinitions: NodeDefinition[] = [
    ...
    { key: 'CHECK_KYC', type: 'CONDITION', label: 'Validar KYC y consentimiento', order: 2, terminal: false, config: {} },
    { key: 'CHECK_FRAUD', type: 'CONDITION', label: 'Evaluar señales de fraude', order: 3, terminal: false, config: {} },
    { key: 'CHECK_AGE', type: 'CONDITION', label: 'Validar elegibilidad', order: 4, terminal: false, config: {} },
    {
      key: 'SCORE',
      type: 'SCORE',
      ...
      config: {
        scoreExpression: {
          op: 'round',
          arg: { op: 'add', args: [{ value: 300 }, { op: 'div', left: { var: 'bureau_score' }, right: { value: 2 } }] },
          precision: 0,
        },
      },
    },
    { key: 'CHECK_RISK', type: 'CONDITION', label: 'Aplicar umbral de riesgo', order: 6, terminal: false, config: {} },
    { key: 'APPROVE', type: 'ACTION', label: 'Aprobar y asignar línea', order: 7, terminal: true, config: {} },
    { key: 'DECLINE_KYC', type: 'ACTION', label: 'Rechazar por KYC', order: 8, terminal: true, config: {} },
    { key: 'MANUAL_REVIEW', type: 'ACTION', label: 'Derivar a revisión manual', order: 9, terminal: true, config: {} },
    { key: 'DECLINE_AGE', type: 'ACTION', label: 'Rechazar por elegibilidad', order: 10, terminal: true, config: {} },
    { key: 'DECLINE_RISK', type: 'ACTION', label: 'Rechazar por riesgo', order: 11, terminal: true, config: {} },
  ];
  ...
  for (const [nodeKey, actionCodes] of Object.entries(nodeActionBindings)) {
    for (let index = 0; index < actionCodes.length; index += 1) {
      await prisma.decisionNodeAction.create({
        data: { nodeId: nodeByKey[nodeKey].id, actionId: actionByCode[actionCodes[index]].id, executionOrder: index + 1 },
      });
    }
  }
  ...
  const edgeDefinitions: EdgeDefinition[] = [
    ...
    { key: 'E_KYC_OK', from: 'CHECK_KYC', to: 'CHECK_FRAUD', priority: 1, default: false, condition: 'KYC_OK' },
    { key: 'E_KYC_FAIL', from: 'CHECK_KYC', to: 'DECLINE_KYC', priority: 999, default: true },
    { key: 'E_FRAUD_CLEAR', from: 'CHECK_FRAUD', to: 'CHECK_AGE', priority: 1, default: false, condition: 'FRAUD_CLEAR' },
    { key: 'E_FRAUD_REVIEW', from: 'CHECK_FRAUD', to: 'MANUAL_REVIEW', priority: 999, default: true },
    { key: 'E_AGE_OK', from: 'CHECK_AGE', to: 'SCORE', priority: 1, default: false, condition: 'ELIGIBLE_AGE' },
    ...
    { key: 'E_RISK_OK', from: 'CHECK_RISK', to: 'APPROVE', priority: 1, default: false, condition: 'RISK_ACCEPTABLE' },
  ...
  for (const edge of edgeDefinitions) {
    ...
    if (edge.condition) {
      await prisma.decisionEdgeCondition.create({
        data: { edgeId: row.id, conditionId: conditionByCode[edge.condition].id, evaluationOrder: 1 },
      });
    }
  }

  return { conditionDefinitions, actionDefinitions, nodeDefinitions, nodeActionBindings, edgeDefinitions, conditionByCode, actionByCode, nodeByKey, edgeRows };
}
