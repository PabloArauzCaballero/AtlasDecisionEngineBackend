...
const JS_FORBIDDEN: ForbiddenPattern[] = [
  { pattern: /\brequire\s*\(/, code: 'JS_REQUIRE', message: 'require() is not allowed in imported code' },
  { pattern: /\bimport\s+/, code: 'JS_IMPORT', message: 'import statements are not allowed in imported code' },
  { pattern: /\bprocess\s*\./, code: 'JS_PROCESS', message: 'process access is not allowed in imported code' },
  { pattern: /\beval\s*\(/, code: 'JS_EVAL', message: 'eval() is not allowed in imported code' },
  { pattern: /\bnew\s+Function\s*\(/, code: 'JS_NEW_FUNCTION', message: 'new Function() is not allowed in imported code' },
  { pattern: /\bchild_process\b/, code: 'JS_CHILD_PROCESS', message: 'child_process access is not allowed in imported code' },
  { pattern: /\bfs\s*\./, code: 'JS_FS', message: 'filesystem access is not allowed in imported code' },
  { pattern: /\b__proto__\b|\bconstructor\s*\.\s*constructor\b/, code: 'JS_PROTO_ESCAPE', message: 'prototype-chain escape patterns are not allowed in imported code' },
];

const PYTHON_FORBIDDEN: ForbiddenPattern[] = [
  { pattern: /^\s*import\s+/, code: 'PY_IMPORT', message: 'import statements are not allowed in imported code' },
  { pattern: /^\s*from\s+\S+\s+import\b/, code: 'PY_IMPORT_FROM', message: 'import statements are not allowed in imported code' },
  { pattern: /\bsubprocess\b/, code: 'PY_SUBPROCESS', message: 'subprocess access is not allowed in imported code' },
