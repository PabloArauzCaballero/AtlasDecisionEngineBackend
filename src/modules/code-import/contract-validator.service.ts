...
export class ContractValidatorService {
  validate(contract: MetadataContract, language: ImportLanguage, scriptBody: string): LineIssue[] {
    ...
    if (!contract.inputs.length && !contract.outputs.length) {
      issues.push(
        this.issue('CONTRACT_EMPTY', 'The contract declares no inputs and no outputs', 'WARNING'),
      );
    }
    ...
    if (
      contract.primaryOutputId &&
      !contract.outputs.some((output) => output.id === contract.primaryOutputId)
    ) {
      issues.push(
        this.issue(
  ...
  private validateVariableShape(
    ...
    if (!variable.id || !ID_PATTERN.test(variable.id)) {
      issues.push(
        this.issue('CONTRACT_ID_INVALID', `Variable id "${variable.id}" must match ${ID_PATTERN}`),
      );
      return;
    }
    if (seen.has(variable.id)) {
      issues.push(
        this.issue(
          ...
          `Variable id "${variable.id}" is declared more than once`,
        ),
      );
    }
    ...
    if (!variable.name || !variable.name.trim()) {
      issues.push(
        this.issue('CONTRACT_NAME_MISSING', `Variable "${variable.id}" has no display name`),
      );
    }
    ...
    if (typeof variable.required !== 'boolean') {
      issues.push(
        this.issue(
          ...
          `Variable "${variable.id}" must declare "required" as a boolean`,
        ),
      );
    }
  ...
  private isReferenced(
    ...
    const readPattern = new RegExp(
      `variables\\s*(?:\\.\\s*${escaped}\\b|\\[\\s*['"]${escaped}['"]\\s*\\])`,
    );
    const writePattern = new RegExp(
      `(?:return\\s*\\{[^}]*\\b${escaped}\\b|result\\s*\\.\\s*${escaped}\\s*=|result\\s*\\[\\s*['"]${escaped}['"]\\s*\\]\\s*=)`,
      's',
    );
    return mode === 'read' ? readPattern.test(scriptBody) : writePattern.test(scriptBody);
  }
