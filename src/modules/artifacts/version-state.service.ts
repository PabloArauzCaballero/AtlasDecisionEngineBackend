...
const allowedTransitions: Record<VersionStatus, VersionStatus[]> = {
  ...
  DEPLOYED_TO_DEV: ['DEPLOYED_TO_TEST', 'DEPLOYED_TO_STAGING', 'APPROVED', 'RETIRED'],
  // TEST → PROD sigue permitido además de TEST → STAGING → PROD. STAGING es un
  // escalón que se ofrece, no un peaje nuevo: convertirlo en obligatorio habría
  // invalidado de golpe promociones que hoy son legales y están auditadas.
  DEPLOYED_TO_TEST: ['DEPLOYED_TO_STAGING', 'DEPLOYED_TO_PROD', 'APPROVED', 'RETIRED'],
  DEPLOYED_TO_STAGING: ['DEPLOYED_TO_PROD', 'APPROVED', 'RETIRED'],
